...
音楽制作6年の経験から、依頼者が「落ち着いた雰囲気でBPM90前後の楽曲」といった感覚的なニーズで音楽家を探せないことを課題と特定。既存プラットフォームはジャンル・価格帯の粗いフィルタしかなく、双方のミスマッチが常態化していた。

楽曲を**音響解析（librosa）でBPM・キー・ジャンルに数値化**し、**LLMで自然言語マッチング**することで、感覚的なニーズと楽曲を直接つなぐプラットフォームを開発した。
...
|---------|------|
| 要件定義 | シニアエンジニアと共同 |
| DB設計 / Rails 7 API / RSpec 68ファイル | ソロ |
| Next.js 15 App Router / TypeScript strict | ソロ |
| Python librosa解析 / OpenAI GPT-4o-mini連携 | ソロ |
| Docker Compose / EC2 / GitHub Actions CI/CD | ソロ |
